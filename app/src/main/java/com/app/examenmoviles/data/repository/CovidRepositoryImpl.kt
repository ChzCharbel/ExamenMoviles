...
class CovidRepositoryImpl @Inject constructor(
     ...
     */
    override suspend fun refreshTopCountries(): Result<List<CountryCovid>> {
        return fetchTopCountriesFromApi()
    }
    ...
    private suspend fun fetchTopCountriesFromApi(): Result<List<CountryCovid>> {
        return try {
            ...
            val countriesData = coroutineScope {
                CovidConstants.DEFAULT_TOP_COUNTRIES.map { countryName ->
                    async {
                        try {
